...
exports.AllAds = async (req, res) => {
  ...
  try {
    if (searched && searched.trim() !== "") {
      filter = {
        $or: [{ name: { $regex: searched, $options: "i" } }, { position: { $regex: searched, $options: "i" } }, { provider: { $regex: searched, $options: "i" } }],
      };
    }
    const totalAdsCount = await Ads.countDocuments(filter);
    const adsListResponse = await Ads.find(filter).skip(limit * start)
    .limit(limit);
    if (!adsListResponse) {
